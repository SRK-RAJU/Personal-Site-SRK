...
export default function NewPostPage() {
  ...
  const [imageRenderer, setImageRenderer] = useState('plantuml-kroki');
  ...
  const handleGenerateFeatureImage = async () => {
    ...
    try {
      ...
      setImageMessage(`Architecture image rendered with ${result.model_used || imageRenderer}.`);
    } catch (err: any) {
      ...
      <form onSubmit={handleSubmit} className="dashboard-card space-y-6">
        ...
        <div className="rounded-lg border border-cyan-200 bg-cyan-50 p-4 dark:border-cyan-800 dark:bg-cyan-900/20">
          ...
          <p className="mt-1 text-xs text-cyan-800 dark:text-cyan-300">
            Render one enterprise architecture image from the tool and module details below.
          </p>
          ...
          <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <div>
              <label htmlFor="imageRenderer" className="mb-2 block text-xs font-semibold text-slate-900 dark:text-white">
                Image renderer
              </label>
              <select
                id="imageRenderer"
                value={imageRenderer}
                onChange={(event) => setImageRenderer(event.target.value)}
                className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-cyan-500 dark:border-slate-600 dark:bg-slate-800 dark:text-white"
              >
                <option value="plantuml-kroki">PlantUML + Kroki (enterprise diagram)</option>
              </select>
            </div>
          </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            >
              {isGeneratingImage ? 'Rendering image...' : 'Render architecture image'}
            </button>
