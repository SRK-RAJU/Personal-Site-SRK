...
export default function DashboardSettingsPage() {
  ...
  useEffect(() => {
    setAdminEmail(process.env.NEXT_PUBLIC_ADMIN_EMAILS || process.env.ADMIN_EMAILS || '');
  }, []);
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div className="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800 dark:border-blue-900/50 dark:bg-blue-950/20 dark:text-blue-200">
          ...
          <div>
            ...
            <p className="mt-1">Set the admin email in your environment variables and redeploy for automatic admin recognition.</p>
          </div>
        </div>

        <div className="mt-6 rounded-xl border border-slate-200 p-4 dark:border-slate-700">
          <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Configured admin emails</p>
          <p className="mt-2 break-all text-sm text-slate-600 dark:text-slate-300">{adminEmail || 'No admin emails configured yet.'}</p>
        </div>
      </div>
    </div>
  );
