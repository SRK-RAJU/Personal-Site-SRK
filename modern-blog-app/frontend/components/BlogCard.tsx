      ...
      <Link href={`/blog/${slug}`}>
        <div className={`h-full bg-gradient-to-br from-white/5 to-white/0 dark:from-slate-800/20 dark:to-slate-900/10 rounded-xl border border-violet-500/20 dark:border-violet-500/30 overflow-hidden hover:border-violet-400/50 dark:hover:border-violet-400/60 transition-all duration-300 hover:shadow-2xl hover:shadow-violet-500/30 flex flex-col backdrop-blur-sm ${
          ...
          <div className="p-6 flex-1 flex flex-col">
            ...
            {/* Excerpt */}
            <p className="text-slate-800 dark:text-slate-200 text-sm mb-4 line-clamp-2 flex-1">
              {excerpt || content?.substring(0, 100) + '...'}
            </p>

            {/* Meta info */}
            <div className="flex flex-wrap items-center gap-4 pt-4 border-t border-violet-500/20 dark:border-violet-500/30 text-sm text-slate-800 dark:text-slate-100 group-hover:text-violet-300 dark:group-hover:text-violet-200 transition-colors">
            ...
            {tags && tags.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {tags.slice(0, 3).map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex px-2 py-1 rounded text-xs bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-200"
                  >
                    #{tag}
                  </span>
                ))}
